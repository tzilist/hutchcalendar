defmodule HutchCalendar.ReservationService do
  ...
  def query_availability(time_start, time_end, room_id) do
    ...
    query = from res in "reservations",
      where: res.conference_room_id == ^room_id and res.time_start < type(^time_end, Ecto.DateTime) and res.time_end >= type(^time_end, Ecto.DateTime),
      or_where: res.id == ^room_id and res.time_end > type(^time_start, Ecto.DateTime) and res.time_start < type(^time_start, Ecto.DateTime),
